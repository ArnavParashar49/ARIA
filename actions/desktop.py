...
def clean_desktop(params: dict | None = None) -> str:
    ...
    candidates = [
        item for item in desktop.iterdir()
        if item.is_file()
        and not item.name.startswith(".")
        and item.suffix.lower() not in skip_exts
    ]

    if not proceed:
        if not candidates:
            return "Desktop has nothing to archive."
        return cg.needs_confirm(
            "desktop_clean",
            (
                f"Archive {len(candidates)} loose file(s) on the Desktop into "
                f"'Desktop Archive {today}'."
            ),
            {"action": "clean"},
            ask=f"Should I clean the desktop and archive {len(candidates)} file(s)?",
        )

    p = cg.merge_params(params, stored) if stored else params
    if p.get("action") != "clean":
        return "CANCELLED: Desktop clean cancelled."
